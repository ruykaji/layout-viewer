...
void
save_as(const std::filesystem::path& file_path, const Tp* data, const std::vector<std::size_t>& shape)
{
  ...
  const std::size_t fixed_header_bytes = 6 /*magic*/ + 2 /*version*/ + 2 /*header_len_field*/;
  const std::size_t padding            = (16 - ((header.size() + fixed_header_bytes + 1) % 16)) % 16;
  const std::size_t header_length      = header.size() + padding + 1;
  ...
  /** Write the data */
  out_file.write(reinterpret_cast<const char*>(data), data_length * sizeof(Tp));
