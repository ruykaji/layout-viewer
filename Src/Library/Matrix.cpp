...
uint8_t Matrix::get_at(const uint8_t x, const uint8_t y, const uint8_t z) const
{
...
void Matrix::set_at(const uint8_t value, const uint8_t x, const uint8_t y, const uint8_t z)
{
    if (x >= m_shape.m_x || y >= m_shape.m_y || z >= m_shape.m_z) {
        throw std::out_of_range("Out of range");
    }
...
void Matrix::clear() noexcept(true)
{
    m_shape = Shape { 0, 0, 0 };
